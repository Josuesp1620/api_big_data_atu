...
def get_data_body(request):
    ...
    return {    
        ...
        "max_suma_viajes": max_suma_viajes,
    }
...
def get_multiple_data_centroid():
    return

@app.route('/filter_data', methods=['POST'])
def filter_data():
    data_body = get_data_body(request)
    field = [key for key, value in data_body["query_target"].items() if len(value) != 0 and key.startswith("taz_") and key.endswith("_o")][0]
    ...
    print(results_queries_initial)
    # print(get_one_data_centroid(query_target=data_body["query_target"], query_type=data_body["query_type"], field=field))
    
    # # Función para obtener los datos del API de centroides
    # def get_centroid_data(item, field):
    #     target = get_data_centroid_api(body={"taz":item[field.replace('_o', '_d')]}, tag_name="data_centroid", name_name=query_type)[0]        
